...
class ShopListWidget extends StatefulWidget {
  ...
  @override
  ShopListWidgetState createState() => ShopListWidgetState();
}
...
class ShopListWidgetState extends State<ShopListWidget> {
  ...
  List<ToDo_Item> todoListShop;

  List<ToDo_Item> get todoListItems => todoListShop;
  ...
  void initState() {
    ...
    todoListShop = List<ToDo_Item>();
    databaseHelper = DatabaseHelper();

    databaseHelper.allTodosShop().then((mapList) {
      for (Map map in mapList) {
        todoListShop.add(ToDo_Item.fromMap(map));
  ...
  Future<int> _ekle(ToDo_Item item) async {
    ...
    await databaseHelper.addTodoShop(item).then((addInt) {
      setState(() {
        //todoList.add(item);
        item.id = addInt;

        todoListShop.insert(0, item);
        _listKey.currentState.insertItem(indexList++);
  ...
  Future<int> _update(ToDo_Item item, int index) async {
    ...
    var result = await databaseHelper.allTodosShop();
    // print(result);
    // databaseHelper.deleteTodo(4); // silme
  }
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        ...
        body: AnimatedList(
          ...
          initialItemCount: todoListShop?.length ?? 0,
          itemBuilder: (context, index, animation) {
            return _buildItem(todoListShop[index], animation, index);
          },
        ),
        drawer: Container(
          width: 210,
            ...
            children: <Widget>[
              ...
              Row(
                ...
                children: <Widget>[
                  GestureDetector(
                    onTap: () {
                      txt.text = todoListShop[index].title;
                      txt1.text = todoListShop[index].text;
                      txt2.text = todoListShop[index].other;

                      title = todoListShop[index].title;
                      note = todoListShop[index].text;
                      other = todoListShop[index].other;
...
///// Iterator Design Pattern function . Class is at the 632. line.
  ...
  void _removeSingleItems(int removeAt) {
    ...
    indexList--;
    ToDo_Item removedItem = todoListShop.removeAt(removeIndex);
              ...
              actions: [
                ...
                CupertinoDialogAction(
                    ...
                    onPressed: () {
                      ...
                      print(index2);
                      print(indexList);
                      if (todoListShop.isNotEmpty) {
                        print(todoListShop[index2]);

                        databaseHelper
                            .deleteTodo(todoListShop[index2].id)
                            .then((deletedId) {
                          setState(() {
                            todoListShop.removeAt(indexList);
                          });
                        });
                      }
