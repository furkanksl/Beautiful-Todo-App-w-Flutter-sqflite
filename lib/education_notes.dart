...
class EducationListWidget extends StatefulWidget {
  ...
  @override
  EducationListWidgetState createState() => EducationListWidgetState();
}
...
class EducationListWidgetState extends State<EducationListWidget> {
  ...
  List<ToDo_Item> todoListEdu;

  List<ToDo_Item> get todoListItems => todoListEdu;
  ...
  void initState() {
    ...
    todoListEdu = List<ToDo_Item>();
    databaseHelper = DatabaseHelper();


    databaseHelper.allTodosEdu().then((mapList) {
      for (Map map in mapList) {
        todoListEdu.add(ToDo_Item.fromMap(map));
  ...
  Future<int> _ekle(ToDo_Item item) async {
    ...
    await databaseHelper.addTodoEdu(item).then((addInt) {
      setState(() {
        //todoList.add(item);
        item.id = addInt;

        todoListEdu.insert(0, item);
        _listKey.currentState.insertItem(indexList++);
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        ...
        body: AnimatedList(
          ...
          initialItemCount: todoListEdu.length ?? 0,
          itemBuilder: (context, index, animation) {
            return _buildItem(todoListEdu[index], animation, index);
          },
        ),
        drawer: Container(
          width: 210,
            ...
            children: <Widget>[
              ...
              Row(
                ...
                children: <Widget>[
                  GestureDetector(
                    onTap: () {
                      txt.text = todoListEdu[index].title;
                      txt1.text = todoListEdu[index].text;
                      txt2.text = todoListEdu[index].other;

                      title = todoListEdu[index].title;
                      note = todoListEdu[index].text;
                      other = todoListEdu[index].other;
...
///// Iterator Design Pattern function . Class is at the 632. line.
  ...
  void _removeSingleItems(int removeAt) {
    ...
    indexList--;
    ToDo_Item removedItem = todoListEdu.removeAt(removeIndex);
              ...
              actions: [
                ...
                CupertinoDialogAction(
                    ...
                    onPressed: () {
                      ...
                      print(index2);
                      print(indexList);
                      if (todoListEdu.isNotEmpty) {
                        print(todoListEdu[index2]);

                        databaseHelper
                            .deleteTodoEdu(todoListEdu[index2].id)
                            .then((deletedId) {
                          setState(() {
                            todoListEdu.removeAt(indexList);
                          });
                        });
                      }
